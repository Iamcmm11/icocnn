...
### 0.5 2026-03-24 量化调参补充

2026-03-24，`layer2-5` 在保持当前结构骨架不变的前提下，引入了
`ap_fixed` 量化类型，并将默认组合收敛为：
...
同时，本地定点 C 验证显示四层误差已被压缩到较低水平：

- `layer2`: `Max Error = 0.0119269`, `RMSE = 0.00335319`
- `layer3`: `Max Error = 0.0150612`, `RMSE = 0.00344091`
- `layer4`: `Max Error = 0.0137136`, `RMSE = 0.00343121`
- `layer5`: `Max Error = 0.0119314`, `RMSE = 0.00279214`

因此当前阶段结论应更新为：

1. `layer2-5` 已从“首版量化摸底”进入“量化参数基本收敛 + 结构优化继续推进”的新阶段。
2. 量化没有改变主瓶颈类型，`PadIco` 输入端口冲突与局部 `output_tile` 累加调度仍是后续主线。
3. 由于量化释放了较多 `BRAM/DSP` 余量，后续更适合先做有针对性的输入缓冲与局部 banking，而不是立刻做大范围激进并行化。
4. 当前剩余量化问题已经从“大偏差”收敛为“距离严格阈值仍差最后一轮精修”，因此后续调参应以小步精修为主。

### 0.6 量化后的统一下一步

1. 对 `layer2-5`，先在当前默认量化组合周围做小范围精修，优先补齐最后一轮数值闭环。
2. 对 `layer2-5`，在误差稳定后优先处理 `PadIco` 的输入端口冲突，因为它仍是最明确的 `II` 限制来源。
3. 对 `layer2-5`，再利用量化释放的资源余量继续压缩局部 `output_tile` 累加阶段的启动间隔。
4. 对 `layer1`，后续若引入量化或局部 tile 累加结构，应优先复用 `layer2-5` 已验证有效的“先结构化，再定点化，并在量化阶段保留可调位宽接口”这一推进顺序。
...
## 14. 2026-04-07 Layer0 / Layer1 新一轮目标瓶颈映射

在 `layer2-5` 已完成结构级重构之后，`layer0` 与 `layer1` 的下一轮优化目标已经进一步明确，不再建议把两层视为同构对象处理。

### 14.1 Layer0

`layer0` 的主目标仍然是压缩输出后处理开销，而不是引入完整共享主干骨架。

当前代码侧已按这一方向推进为：

1. `PadIco` 改成 `Cin=1, Rin=1` 的 fast path
2. 主卷积先写本地 `local_output`
3. 极点平滑在本地输出帧完成
4. 最后统一写回顶层 `output`

因此，`layer0` 下一轮 HLS 关注点应更新为：

1. 输出后处理相关 loop 的 `Final II`
2. 顶层 `output_r` 相关的端口冲突是否显著减少
3. 极点平滑求和依赖是否因 fast path 而缩短

换言之，`layer0` 的重点是：

`输出后处理端口冲突 > PadIco 极点求和依赖 > 输入侧访问`

### 14.2 Layer1

`layer1` 已不再适合继续沿用“layer0 放大版”写法。当前代码已切换到：

1. `stage_input -> pad_ico -> main_conv_partial_sum -> output_postprocess -> writeback`
2. 局部 `OC_TILE`
3. `ri_partial` 分层部分和
4. 输出极点后处理上移到局部 tile 内完成

因此，`layer1` 当前最值得追踪的瓶颈顺序应更新为：

1. 主 MAC / partial-sum 路径的 loop-carried dependence
2. `PadIco` 输入端口冲突
3. 输出后处理阶段是否仍残留局部 tile 内部的读改写热点

与旧版相比，`layer1` 的优化目标已从：

`全局 output 读写冲突 + 串行 MAC`

转向：

`局部 tile 归并调度 + PadIco 访存组织`

### 14.3 当前阶段性判断

因此，前两层的目标瓶颈映射应统一表述为：

1. `layer0` 重点是输出后处理端口冲突
2. `layer1` 重点是输出端口冲突重构后的主 MAC 累加路径与 `PadIco` 输入端口冲突
3. `layer2-5` 继续作为共享主干结构优化的主承载层

这一定义能够与代码现状保持一致，也更适合论文中“分层异构硬件映射”的叙事闭环。
