...
set "PROJECT=%~4"
if "%PROJECT%"=="" set "PROJECT=layer0_hls_prj"
set "SOLUTION=%~5"
if "%SOLUTION%"=="" set "SOLUTION=sol1"
set "TOP=%~6"
if "%TOP%"=="" set "TOP=conv_ico_layer0"
...
if errorlevel 1 (
    ...
    echo Please source Xilinx environment first, e.g.:
    echo   call ^<Vitis_install^>\settings64.bat
    popd
    exit /b 1
)

where tee.exe >nul 2>nul
if errorlevel 1 (
    set "TEE_DIR="
    for %%R in ("%XILINX_VITIS%" "%XILINX_HLS%" "%XILINX_VIVADO%") do (
        if not defined TEE_DIR if exist "%%~R" (
            for /f "delims=" %%T in ('where /r "%%~R" tee.exe 2^>nul') do (
                if not defined TEE_DIR set "TEE_DIR=%%~dpT"
            )
        )
    )
    if defined TEE_DIR set "PATH=%TEE_DIR%;%PATH%"
)

where tee.exe >nul 2>nul
if errorlevel 1 (
    echo [WARN] tee.exe not found in current PATH.
    echo [WARN] If Vitis launch fails with "tee.exe not recognized", add:
    echo [WARN]   "%XILINX_VITIS%\tps\win64\msys64\usr\bin"
)
