...
static std::string resolve_data_dir(int layer_id) {
...
static void save_flat(const std::string& file, const std::vector<float>& values, const std::string& shape) {
...
int main(int argc, char** argv) {
    ...
    const std::string data_dir = resolve_data_dir(layer_id);
    if (data_dir.empty()) {
    ...
    conv_ico_layer2_5(input, weight, bias, kernel_idx, reorder_idx, out);
    ...
    std::cout << "Saved layer2-5 debug intermediates to: " << out_dir << std::endl;
    return 0;
}
