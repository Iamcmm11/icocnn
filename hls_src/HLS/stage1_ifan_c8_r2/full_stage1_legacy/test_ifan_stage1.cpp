...
#ifdef __HLS_CSIM__
#include "ifan_stage1_engines.cpp"
#include "ifan_stage1.cpp"
#endif
...
static bool init_real_data_from(const std::string &data_dir) {
    ...
    const std::size_t golden_count =
        static_cast<std::size_t>(IFAN_STAGE1_T) * IFAN_BRANCH_CHANNELS * IFAN_R_FULL * IFAN_CHARTS * IFAN_H_R1 * IFAN_W_R1;

    if (!read_flat_file(data_dir + "/stage1_input.txt", &g_input[0][0][0][0][0], input_count)) {
        return false;
    }

    bool ok = true;
    ok = ok && read_flat_file(data_dir + "/final_head_logits.txt", &g_golden[0][0][0][0][0][0], golden_count);
    if (!ok) {
        std::cerr << "Missing or invalid Stage-1 golden file: " << data_dir + "/final_head_logits.txt" << "\n";
    }
...
int main() {
    ...
    if (!real_data) {
        std::cout << "Real Stage-1 data not found; using synthetic test data.\n";
        init_input();
        init_weights();
        init_reorder_tables();
        init_kernel_tables();
    ...
    std::cout << "IFAN Stage-1 HLS test\n";
