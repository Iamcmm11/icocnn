...
def default_output_path(checkpoint_path: str | Path, subset: str, array: str) -> Path:
    ...
    return Path("IFAN_Maba/outputs/stage3/analysis") / f"locata_{subset}_{array}_{stem}.json"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate an IFAN_Maba stage-3 checkpoint on LOCATA Task 1/3/5.")
    parser.add_argument(
        "--checkpoint",
        required=True,
...
def main() -> None:
    ...
    report = {
        ...
        "config": {
            ...
            "temporal_backend": str(config.temporal_backend),
            "maba": config.maba.__dict__,
        },
        **results,
        "model_profile": {
            "trainable_params": int(ifan_model.count_parameters(trainable_only=True)),
            "total_params": int(ifan_model.count_parameters(trainable_only=False)),
        },
