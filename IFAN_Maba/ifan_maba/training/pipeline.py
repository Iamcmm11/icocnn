...
from ..models import IFANModel, IFANModelConfig, MABATemporalConfig, PAPER_IFAN_BRANCH_CHANNELS
...
class IFANTrainingConfig:
    ...
    experiment_name: str = "ifan_maba_stage3"
    output_root: str = "IFAN_Maba/outputs/stage3"
    output_suffix: str = ""
    ...
    def __post_init__(self) -> None:
        self.temporal_backend = str(self.temporal_backend)
        if self.temporal_backend not in {"conv1d", "maba"}:
            raise ValueError(
                f"Unsupported temporal_backend {self.temporal_backend!r}; expected 'conv1d' or 'maba'."
            )
        self.maba = MABATemporalConfig.from_mapping(self.maba)
    ...
    def from_toml(cls, path: str | Path) -> "IFANTrainingConfig":
        ...
        return cls(
            ...
            temporal_backend=str(model.get("temporal_backend", cls.temporal_backend)),
            maba=MABATemporalConfig.from_mapping(
                {
                    "channels": PAPER_IFAN_BRANCH_CHANNELS,
                    "d_model": maba.get("d_model", MABATemporalConfig.d_model),
                    "state_dim": maba.get("state_dim", MABATemporalConfig.state_dim),
                    "conv_kernel": maba.get("conv_kernel", MABATemporalConfig.conv_kernel),
                    "dropout": maba.get("dropout", MABATemporalConfig.dropout),
                    "use_residual": maba.get("use_residual", MABATemporalConfig.use_residual),
                    "use_gate": maba.get("use_gate", MABATemporalConfig.use_gate),
                    "use_state": maba.get("use_state", MABATemporalConfig.use_state),
                }
            ),
    ...
    def model_config(self) -> IFANModelConfig:
        return IFANModelConfig(
            ...
            maba=self.maba.with_channels(PAPER_IFAN_BRANCH_CHANNELS),
        )
        ...
        *,
        ...
        payload = {
            ...
            "model_config": {
                ...
                "temporal_backend": self.model_config.temporal_backend,
                "maba": asdict(self.model_config.maba),
            },
            "metrics": metrics,
        }
